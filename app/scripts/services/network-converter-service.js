...
viewerApp.service('networkConverterService', ['ndexHelper', function (ndexHelper) {
  ...
  var unspeakableNodeLabelHack = function (label) {
    ...
    if (label === 'PROTEIN_ABUNDANCE(HGNC:RPS6KB1,bel:PROTEIN_MODIFICATION(bel:P,bel:T,bel:444))') {
      return 'RPS6KB1 phosphorylated at T444';
    }

    if (label === 'ABUNDANCE(CHEBI:sorafenib tosylate)') {
      return 'sorafenib tosylate';
    }

    if (label === 'KINASE_ACTIVITY(bel:PROTEIN_ABUNDANCE(HGNC:BRAF))') {
      return 'activated BRAF';
    }

    if (label === 'PROTEIN_ABUNDANCE(HGNC:BRAF,bel:PROTEIN_MODIFICATION(bel:P,bel:S,bel:445))') {
      return 'BRAF phosphorylated at S445';
    }

    if (label === 'PROTEIN_ABUNDANCE(HGNC:NRAS,bel:SUBSTITUTION(bel:Q,bel:61,bel:R))') {
      return 'NRAS Q61R';
    }

    if (label === 'KINASE_ACTIVITY(bel:PROTEIN_ABUNDANCE(HGNC:AKT1))') {
      return 'activated AKT1';
    }

    if (label === 'CATALYTIC_ACTIVITY(bel:PROTEIN_ABUNDANCE(HGNC:TSC2))') {
      return 'activated TSC2';
    }

    if (label === 'PATHOLOGY(SDIS:Oncogene induced senescence)') {
      return 'Oncogene induced senescence';
    }

    if (label === 'GTP_BOUND_ACTIVITY(bel:PROTEIN_ABUNDANCE(HGNC:NRAS))') {
      return 'activated NRAS';
    }

    if (label === 'KINASE_ACTIVITY(bel:PROTEIN_ABUNDANCE(HGNC:ERBB4))') {
      return 'activated ERBB4';
    }

    if (label === 'KINASE_ACTIVITY(bel:PROTEIN_ABUNDANCE(PFH:PRKA Family))') {
      return 'activated PRKA Family';
    }

    if (label === 'PROTEIN_ABUNDANCE(HGNC:ERBB4,bel:PROTEIN_MODIFICATION(bel:P,bel:Y))') {
      return 'ERBB4 phosphorylated at Y';
    }

    if (label === 'PATHOLOGY(SDIS:tumor growth)') {
      return 'tumor growth';
    }

    if (label === 'KINASE_ACTIVITY(bel:PROTEIN_ABUNDANCE(HGNC:MAPK1))') {
      return 'activated MAPK1';
    }
  ...
  var getFriendlyTermLabel = function (term, network) {
    if (term.termType === 'BaseTerm') {
      ...
      if (ftname.indexOf('activity') > 0) {
        ...
        if (mod !== undefined) {
          var modCode = getModCode(parameterList);
          mod = mod.toLowerCase();
    ...
    if (represents !== undefined) {
      ...
      if (baseTerm !== undefined) {
        ...
        if (parenIndex > 0) {
          ...
          var functionString = name.substring(0, parenIndex);
...
*/
  ...
  var getEdgeData = function (ndexEdge, ndexNetwork) {
    ...
    for (var idx2 = 0; idx2 < len; idx2++) {
      var supportObj = ndexNetwork.supports[ndexEdge.supportIds[idx2].toString()];
